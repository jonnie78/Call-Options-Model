...
def get_lastest_stock_price(ticker):
    """
    Function to fetch latest stock price of desiered stock
    ticker = Ticker of stock
    """
...
def trading_simulation(S, E, T, mu, sigma, market_price):
    """
    Simple simulation that trades either a long or short strategy based off of user input. The stock is modeled using historical returns and implied volatility
    S = Current stock price
    E = Option strike price
    T = Time to expiry (y)
    mu = Expected rate of return (historical rate of return used)
    sigma = Volatility (implied volatility used)
